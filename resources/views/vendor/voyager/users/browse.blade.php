...
@section('page_header')
    <h1 class="page-title">
        ...
        @if (Voyager::can('add_'.$dataType->name))
            <a href="{{ route('voyager.'.$dataType->slug.'.create') }}" onclick="modalOpen('{{ route('voyager.'.$dataType->slug.'.create') }}'); return false;" id="myModal1" {{--data-toggle="modal" data-target="#myModal"--}} class="btn btn-success">
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <iframe class="frame-class" src="" style="width: 550px; height: 1100px; border : 0px;"></iframe>
                </div>
                        ...
                        <table id="dataTable" class="table table-hover">
                            ...
                            @foreach($dataTypeContent as $data)
                                <tr>
                                    ...
                                    <td>{{ucwords($data->timezone)}}</td>
                                    <td>{{$data->email}}</td>
                                    <td>{{ $data->created_at->setTimezone(\Illuminate\Support\Facades\Auth::user()->timezone)->format('F jS, Y h:i A') }}</td>
                                    <td>
                                   ...
                                   {{-- <td>{{ $data->role ? $data->role->display_name : '' }}</td>--}}
                                    <td class="no-sort no-click">
                                        ...
                                        @if (Voyager::can('edit_'.$dataType->name))
                                            <a  onclick="modalOpen('{{ route('voyager.'.$dataType->slug.'.edit', $data->id) }}'); return false;" href="{{ route('voyager.'.$dataType->slug.'.edit', $data->id) }}" class="btn-sm btn-primary pull-right edit">
